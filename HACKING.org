...
Certain containers (notably, intrusive) are heavily customized using traits classes, and without access to those, one cannot print the containers reliably. Unfortunately, =gdb= is occasionally missing crucial inner typedefs (for reasons unknown to the author). E.g., the =node_traits= typedef is regularly missing from inside various =value_traits= classes. To work around this limitation, the package provides a way to bypass the inner type resolution from inside =gdb= by using the variable =boost.inner_type=.
